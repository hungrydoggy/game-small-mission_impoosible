...
#include "./plugins..relh..relh.pair.mac.macro.h"
...
static void ___MacroLoaderBundleRegisterer___ () {
  ...
  MacroLoaderBundle::default_bundle().registerMacroLoader(
      vector<string>{"RelH.pair", "relh.pair", },
      make_shared<Macro_PluginsRelhRelhPairMac>(),
      true
  );
}
...
string const& Macro_PluginsRelhRelhPairMac::loadable_class () const {
  static string cls = "Macro";
  return cls;
}


bool Macro_PluginsRelhRelhPairMac::isCompatibleLoadable (shared_ptr<NodeLoadable> const& loadable) {
  return dpc<Macro>(loadable) != null;
}


bool Macro_PluginsRelhRelhPairMac::preEmbody (
...
bool Macro_PluginsRelhRelhPairMac::_preLoad (
    LoaderContext& ctx,
    shared_ptr<Node> const& node,
...
bool Macro_PluginsRelhRelhPairMac::_postLoad (
    LoaderContext& ctx,
    shared_ptr<Node> const& node,
    shared_ptr<NodeLoadable> const& loadable
) {
...
static shared_ptr<Content> __on_call_code_4 (
  ...
  static uint code_path_id = PathRegistry::lookUp("plugins/relh/relh.pair.mac");
  auto code_set = CodeSetBundle::default_bundle().getCodeSet(code_path_id);
  unlikely (code_set == null) {
    LOG_ERR("code_set is null --- plugins/relh/relh.pair.mac");
    return null;
  }
...
shared_ptr<Content> Macro_PluginsRelhRelhPairMac::execute (
    shared_ptr<Node> const& node,
    shared_ptr<Content> const& content,
    int code_idx,
  ...
  switch (code_idx) {
    ...
    default:
      LOG_ERR("unknown code_idx %u --- \"plugins..relh..relh.pair.mac.macro.cpp\"", code_idx);
      return null;
  }
}
...
shared_ptr<Format> const&  Macro_PluginsRelhRelhPairMac::_getFormat () {
